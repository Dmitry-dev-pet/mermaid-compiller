...
const TYPE_MATCH_RE = /(?:^|—|-)\s*([a-zA-Z0-9]+)\s*-\s*/;
...
export const stripDiagramTypeFromText = (
  text: string,
...
export const stripInnerBlockLabelFromContextText = (text: string) => {
  if (!text.includes("Контекст") && !text.toLowerCase().includes("context"))
    return text;

  const withBoth = text.replace(
    /—\s*\d+\/\d+\s*-\s*[a-zA-Z0-9]+\s*-\s*/g,
    "— ",
  );
