...
*   **`index.html`:**
    ...
    *   **Элементы UI:**
        ...
        *   Textarea/Pre для отображения исходного кода Mermaid.
...
*   **`app.js`:**
    *   **Управление состоянием:**
        ...
        *   `maxRetries`: Константа (например, 2-3) для ограничения попыток исправления.
    ...
    *   **Функция `callCliproxyApi(promptMessage, contextDiagramCode)`:**
        ...
        *   Отправляет `fetch`-запрос на локальный `cliproxyapi` (например, `http://localhost:port/generate`).
    ...
    *   **Функции обновления UI:**
        ...
        *   `displayRawCode(code)`: Обновляет область отображения исходного кода.
