...
4.  Пользователь выбирает действие:
    * **Chat:** отправляется запрос в LLM (режим `chat`) и добавляется только текстовый ответ (код диаграммы не применяем).
    * **Build:** отправляется запрос в LLM (режим `generateDiagram`) и обновляются редактор/превью по извлечённому Mermaid коду.
5.  Ответ ИИ (текст) добавляется в чат (код вырезается из текста сообщения для чистоты).
