...
*   **Функции:** Подсветка синтаксиса Mermaid (PrismJS + кастомная грамматика), номера строк, поддержка темной темы (One Dark), копирование в буфер, ручной **Snapshot**.
...
*   **Сущности:**
    ...
    *   **TimeStep:** шаг истории, создаётся на **явные действия** (chat/build/fix/analyze/recompile/manual_edit и т.п.).
    *   **DiagramRevision:** ревизия Mermaid-кода, создаётся только когда диаграмма меняется.
*   **Manual edit:** обычные ручные правки обновляют текущую ревизию без создания шага; **Snapshot** создает шаг `manual_edit`.
...
*   **UI:** при наличии Mermaid-блоков в Markdown во втором окне появляются вкладки с содержимым каждого блока; невалидные Mermaid-блоки заменяются inline-ошибкой на месте блока.
...
*   **Вкладки:** в редакторе и превью отображаются отдельные вкладки для Chat, Build, Analyze и Fix.
