...
*   **Вход:** Текстовый промпт в `ChatColumn`.
*   **Процесс:** Запрос к LLM с контекстом документации (`docsContextService`) -> парсинг ответа -> валидация.
...
*   **Триггер:** Кнопка "Fix Syntax" (или автоматически при фатальной ошибке генерации).
