...
*   Обеспечить модульность и поддерживаемость кода (JavaScript/ES6+ модули).
...
*   **Механизм повторных попыток:** Система должна предпринимать до 5 попыток исправления (значение лимита может быть конфигурируемым; по умолчанию 5). Если диаграмма остаётся невалидной после этих попыток, пользователь будет проинформирован о неудаче.

### ФТ-006: Отображение визуализации и кода
*   **Описание:** Пользовательский интерфейс должен отображать визуальное представление диаграммы Mermaid и понятные статусные сообщения о процессе генерации и исправления.
...
### ФТ-007: Выбор модели LLM
*   **Описание:** Пользователь может выбрать конкретную модель LLM из списка доступных через `cliproxyapi`.
*   **Фильтрация:** Поддержка фильтрации моделей по вендору (GPT, Gemini, GLM и др.).
*   **Автовыбор:** Логика автоматического выбора лучшей модели (например, последней версии GPT-4) при отсутствии явного выбора.

### ФТ-008: Интеграция с документацией Mermaid
*   **Описание:** Приложение может автоматически подтягивать релевантные фрагменты документации Mermaid для улучшения качества генерации и стилизации диаграмм.
*   **Механизм:** Бэкенд (Node.js) предоставляет API `/docs/search`, который ищет по локальной документации.
...
*   Интерфейс должен быть реактивным и не блокироваться во время запросов к API.
...
*   Наличие модальных окон для просмотра деталей (промптов, стадий генерации, кода) улучшает UX.

### НФТ-003: Модульность и Архитектура
*   **Frontend:** Модульная архитектура на ES6 modules (`main.js`, `api.js`, `state.js`, `ui.js`, `workflows.js`, `modelManager.js`, `prompts.js`, `validation.js`).
*   **Backend:** Node.js (Express) для раздачи статики и проксирования запросов к документации.
*   **State Management:** Паттерн Pub/Sub для управления состоянием и реактивности UI.

### НФТ-004: Технологический стек
*   **Frontend:** Vanilla JS (ES6+), Mermaid.js.
*   **Backend:** Node.js, Express, adm-zip, node-fetch.