...
- показывать операционные логи шагов (тайминг + контекст messages/docs),
...
- В notebook режиме чат и логи могут быть контекстными (main vs per-block), а whiteboard хранит отдельные сцены по блокам.
- Таймаут LLM настраивается в UI и применяется ко всем режимам.
- Проекты (сессии) имеют редактируемое название; при дефолтном названии оно может авто-обновиться после первого Chat.
- UI-контролы и переключатели режимов унифицированы между колонками.

---

Обновлено: 2026-01-23. Согласовано с текущей реализацией (notebook contexts, preview surfaces, whiteboard bundle).
